;(() => {
    ...
    mounted() {
      new IntersectionObserver(
        entries => {
          ...
          document
            .querySelector('#home')
            .classList.toggle('active', isIntersecting)
    ...
    template: `
      <section :id="id">
        <div class="container">
          ...
          <div class="row">
            <slot v-bind="methods" :data="data">{{data}}</slot>
          </div>
        </div>
      </section>
      `,
    data() {
      return { id: this.$options.name, data: null, methods: null }
    },
    mounted() {
      this.$parent.sectionObserver.observe(this.$el)
    },
  })

  // Section#about
  Vue.component('about', { extends: Section })
  ...
  // Section#experience
  Vue.component('experience', { extends: Section })
  Vue.component('work', {
    props: ['data'],
    computed: {
      ...Object.fromEntries(
  ...
  // Section#portfolio
  Vue.component('portfolio', { extends: Section })
  Vue.component('card', {
    props: ['data'],
    computed: {
      ...Object.fromEntries(
  ...
  new Vue({
    ...
    created() {
      ...
      $this.sectionObserver = new IntersectionObserver(
        entries => {
          // Find the first section in viewport
          entries.forEach(entry => {
            const { target, intersectionRatio, isIntersecting } = entry
            $this.observed[target.getAttribute('id')] = intersectionRatio > 0.3
            target.classList.toggle('active', isIntersecting)
          })
          const highlighted = Object.keys($this.observed).find(
            key => $this.observed[key] === true
          )
          ...
          if (highlighted) {
            ...
            document
              .querySelector(`nav [href="#${highlighted}"]`)
    ...
    mounted() {
      fetch(
        ...
        .then(res => res.json())
        .then(res => {
          // Parse data, el[0] stands for the type
